...
public class SmartScriptLexer {
	...
	private void dealWithTag() {
		...
		if (takeEnd()) return;
		if (takeEquals()) return;
		if (takeVariable()) return;
		if (takeFunction()) return;
		if (takeOperator()) return;
		if (takeNumber()) return;
		if (takeString()) return;
	...
	private boolean takeVariable() {
		if (Character.isLetter(data[currentIndex])) {
			...
			if (potential.contains("$}")) {
				currentIndex = triedIndex;
				potential = giveUntil('$');
			} else if (potential.contains("\r")) {
				currentIndex = triedIndex;
				potential = giveUntil('\r');
			}
			checkName(potential);
	...
	/**
	 * Auxiliary method used to process numbers, both integers and doubles, in the
	 * tag mode.
	 * 
	 * @return true, if the word is consumed
	 */
