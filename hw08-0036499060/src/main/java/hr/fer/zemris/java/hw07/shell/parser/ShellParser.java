...
/**
 * Syntactical and lexical analyzer combination for the shell. Default parser
 * mode is argument mode.
 * 
 * @author MarinoK
 */
public class ShellParser {
	...
	public ShellParser(ParserMode mode) {
		if (!mode.equals(ParserMode.LINE_MODE)) {
	...
	/**
	 * Method used argument and mass parser mode. Used to separate the arguments for
	 * shell commands.
	 * 
	 * @param arguments
	 *            as a string
	...
	public List<String> prepareArguments(String arguments) {
		...
		while (currentIndex < data.length) {
			...
			if (data[currentIndex] == '"') {
				if (mode.equals(ParserMode.MASS_MODE) && (prepared.size() > 3)) {
